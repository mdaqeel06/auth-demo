...
import { RESEND_EMAIL_TIME_LIMIT } from "../constants";

const VerifyEmailPrompt = ({ sentEmailId }) => {
  const [timer, setTimer] = useState(RESEND_EMAIL_TIME_LIMIT);
  ...
  const handleResendEmail = async () => {
    try {
      const response = await axiosInstance.post(
        `/resend-email?emailId=${sentEmailId}`
      );
      if (response.status === "OK") {
        setTimer(RESEND_EMAIL_TIME_LIMIT);
      }
    } catch (e) {
      console.error(e);
    }
  ...
  return (
    <div className="w-50 text-center">
      ...
      <p>
        ...
        <span className="text-success">{sentEmailId}</span> with a verification
        link. Please click the link to verify your account.
      </p>
      <p>The verification link can be resent in {timer} seconds.</p>
      <button
